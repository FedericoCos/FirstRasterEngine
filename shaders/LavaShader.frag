...
vec3 getDiffuse(vec3 light_dir, vec3 light_color, vec3 normal, float distance){
    float attenuation = 1.0 / (0.032 * distance * distance);
...
void main() {
    ...
    for(int i = 0; i < 2; i++) {
        vec3 direction_to_light = (ubo.spot_lights[i].position.xyz - position_world.xyz);
        float distance = length(direction_to_light);
        ...
        vec3 light_dir = normalize(direction_to_light);
        float theta = dot(light_dir, normalize(-ubo.spot_lights[i].direction.xyz));
        // Smooth edge transition
        float smoothEdge = smoothstep(ubo.spot_lights[i].outerCutOff, ubo.spot_lights[i].cutOff, theta);
        
        vec3 light_color = ubo.spot_lights[i].color.xyz * ubo.spot_lights[i].color.w;
        vec3 diffuse_light = getDiffuse(light_dir, light_color, normal, distance) * smoothEdge;

        vec3 specular_light = getSpecular(light_dir, light_color, normal, view_direction) * smoothEdge;
    ...
    // Glowing effect
    float time = ubo.time * 0.1;
    float flow_noise = sin(fragTexCoord.x * 10.0 + time * 0.5) * 0.5 + 0.5;

    vec3 lavaColor = mix(vec3(1.0, 0.1, 0.0), vec3(1.0, 0.6, 0.0), flow_noise);
    vec4 emission = vec4(lavaColor, 1.0) * (flow_noise * 2);

    float edgeGlow = max(0.0, dot(normal, view_direction)); 
    vec4 glow = vec4(1.0, 0.5, 0.0, 1.0) * pow(edgeGlow, 3.0); 

    vec4 texColor = texture(texSampler, fragTexCoord + vec2(flow_noise, time * 0.1));
    
    if(length(texColor.rgb) > 0.8){
        emission *= 1.5;
    }


    outColor = texColor * vec4((total_light + ambient_light) * fragColor, 1.0) + emission + glow;
}
